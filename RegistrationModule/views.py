...
def putStudentData(request):
    ...
    stu.save()
    user=User(username=t_stu_email,email=t_stu_email)
...
def studentCourseDisplay(request):
    if request.user.is_authenticated:
        c={}
        course_list=Courses.objects.order_by('c_id')
        enrolled_cs_list=StudentCourse.objects.filter(student_email=request.user.username)
        enrolled_list=[cs.c_id for cs in enrolled_cs_list]
        c['course_list']=course_list
        c['enrolled_list']=enrolled_list
        
        return render(request,'StudentCourseDisplay.html',c)
    else:
        return HttpResponseRedirect('/')

def addStudentCourse(request):
    t_c_id=request.GET.get('c_id',default=None)
    t_student_email=request.user.username
    sc=StudentCourse(c_id=t_c_id,student_email=t_student_email)
    sc.save()
    return HttpResponseRedirect('StudentCourseDisplay.html')


def removeStudentCourse(request):
    t_c_id=request.GET.get('c_id',default=None)
    t_student_email=request.user.username
    sc=StudentCourse.objects.get(c_id=t_c_id,student_email=t_student_email)
    sc.delete()
    return HttpResponseRedirect('StudentCourseDisplay.html')
...
def putTeacherData(request):
    ...
    tchr.save()
    user=User(username=t_tchr_email,email=t_tchr_email )
    user.set_password(t_password)
    user.save()
